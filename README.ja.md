...
Vendor したソースは **Xiph.Org の GitHub tag**(`opus-codec.org` の公式 tarball と同一)を基準に同期する。想定するフローは:

```sh
# 新しい tag があるか確認(変更はしない)
...
> `sync_opus.py` および対応する CI ワークフローは**未実装**。自動化レベル(L1 = 通知のみ / L2 = 自動 PR)は初回リリース前に確定する。設計は [SPEC.ja.md「リリースフロー」節](SPEC.ja.md#リリースフロー) 参照。
